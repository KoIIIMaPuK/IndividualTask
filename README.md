# v0.19.8 — 01.12.2024
## Нововведения
- Убрал баг с чтением файла 
    - При первом открытии файл не читался и в консоль ничего не выводилось 
    - Забыл указать путь к файлу для чтения, в то время, когда добавил его только для записи

- Сделал нормальное чтение файла
- Добавил поток для SClient
- Изменил значение переменной `ServiceCode` в `SService` с `std::uint64t` на `std::string`. 

- Вывод в `Report.txt` осуществляется, но плохо

## Планируется
- Изменить чтение файлов.
 - Идет чтение только с SClient, когда нужно брать информацию с SService и SUsage для получения информации по поводу услуг
...
- Добавил файл `Action.txt`, в котором записываются действие и время этого действия:
 - На данный момент записывются лишь запись и чтение

- Добавил файл `Reoprt.txt` для дальнейшего использования в индивидуальном задании.
 - Нужен для вывода отработанной информации
 - Скорее всего "с изолентой" сделаю вывод
...
## v0.19.4 — 01.12.2024
### Нововведения
- Добавил файл `Param.ini` для создания параметров и выполнения поставленной индивидуальной задачи.
 - На данный момент там ничего нету
 - Заготовки чтения из этого файла

- Добавил в перечесление `enum Action` новое действие: `ATASK`, для выполнения индивидуальной задачи.
 - На данный момент выводит ошибку, ибо логику не добавил.

- Заготовка для фильтрации клиентов.

### Планируется
- Улучшить фильтрацию клиентов
- Улучшить чтение из файла `Param.ini` хотябы с костылём
- Сделать вывод задаания в `Report.txt` хотябы с костылём
